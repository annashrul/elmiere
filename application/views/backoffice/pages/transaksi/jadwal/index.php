			...
			<div class="modal-body">
				<div class="row mb-3">
					...
					<div class='col-md-4 mb-3'>
						<?php $label = 'jam_masuk'; ?>
						<label for="nameBackdrop" class="form-label"><?= getLabel($label) ?></label>
						<input type="time" name="<?= $label ?>" id="<?= $label ?>" class="form-control">
					</div>
					<div class='col-md-4 mb-3'>
						<?php $label = 'jam_keluar'; ?>
						<label for="nameBackdrop" class="form-label"><?= getLabel($label) ?></label>
						<input type="time" name="<?= $label ?>" id="<?= $label ?>" class="form-control">
					</div>
					...
					<div class='col-md-6'>
						<?php $this->load->view($this->semester) ?>
					</div>
					<div class='col-md-6'>
...
//    }
	...
	formInput.validate({
		rules: {
			...
			'jam_masuk': {required: true},
			'jam_keluar': {required: true},
	...
	function ubah(id) {
		ajaxData(base_url + urlLocal + urlEdit, {id: id}, (res) => {
			if (res.status) {
				...
				setIdSelect("ajaran", data['ajaran']);
				setIdSelect("semester", data['semester']);
				setIdSelect("pengajar", data['id_guru']);
				setIdSelect("keahlian", data['id_keahlian']);
				setIdSelect("kelas", data['id_kelas']);
				attrId("tanggal").val(data['tanggal']);
				attrId("jam_masuk").val(data['jam_masuk']);
				attrId("jam_keluar").val(data['jam_keluar']);
				// let newPelajaran = data['pelajaran'].split(",")
				// setValueSelect('pelajaran', data['pelajaran'])
			} else {
				notif(strFailed);
			}
		});
