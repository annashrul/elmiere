		...
		<form class="modal-content form_input" id="form_input">
			...
			<div class="modal-body">
				<div class="row mb-3">
					...
					<div class='col-md-12 mb-3'>
						<?php $label = 'link_website'; ?>
						<label for="<?= $label ?>" class="form-label"><?= getLabel($label) ?></label>
						<input type="text" name="<?= $label ?>" id="<?= $label ?>" class="form-control">
					</div>

					<div class='col-md-12'>
						<?php $label = 'link_image'; ?>
	...
	function ubah(id) {
		ajaxData(base_url + urlLocal + urlEdit, {id: id}, (res) => {
			if (res.status) {
				...
				attrId('link_image').val(data['image']);
				attrId("link_website").val(data['link']);
			} else {
				notif(strFailed);
			}
		});
